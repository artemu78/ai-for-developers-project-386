...
Образ собирает фронтенд и запускает единый Node.js-сервис, который раздаёт
интерфейс и API по префиксу `/api`. Сервис слушает порт из переменной окружения
`PORT`.

```bash
docker build -t call-calendar .
docker run --rm -p 3000:3000 -e PORT=3000 call-calendar
```
